...
def plot_raster(spike_times_E, clusters_E, spike_times_I, clusters_I, sim_params,
    ...
    for section_name, params in sections:
        ax_info.text(0.07, y, section_name,
                     transform=ax_info.transAxes,
                     fontsize=15, fontweight="bold", color="#2166AC", va="top")
        y -= 0.047
        for k, v in params.items():
            ax_info.text(0.07, y, f"{k}", transform=ax_info.transAxes,
                         fontsize=15, color="#444444", va="top")
            ax_info.text(0.93, y, f"{v}", transform=ax_info.transAxes,
                         fontsize=15, color="#111111", va="top", ha="right",
...
def sim(sim_params, neuron_params, syn_params):
    ...
    n_stim_steps = int(sim_params["sim_time"] / sim_params["stim_duration"])
    ...
    print(f"  Channel 0 exc IDs: {sub_pops[0]['exc'][0]} to {sub_pops[0]['exc'][-1]}")
    print(f"  Channel 9 exc IDs: {sub_pops[9]['exc'][0]} to {sub_pops[9]['exc'][-1]}")


    # ── Create one inhomogeneous Poisson generator per channel ───────
    for k in range (sim_params["N_C"]):
        rate_times = []
        rate_values = []

        for step_i in range(n_stim_steps):
            t_start = step_i* sim_params["stim_duration"] + 0.1
            t_end = (step_i+1)* sim_params["stim_duration"]

            rate_times.append(t_start)
            if active_channels[step_i] == k:
                rate_values.append(nu_stim)
            else:
                rate_values.append(0.0)
            
            if step_i < n_stim_steps - 1:
                rate_times.append(t_end)
                rate_values.append(0.0)

        stim_gen = nest.Create("inhomogeneous_poisson_generator", params={"rate_times": rate_times, "rate_values": rate_values})

        # Connect to this channel's E and I neurons
        nest.Connect(stim_gen, sub_pops[k]["exc"], "all_to_all",
                    {"synapse_model": "static_synapse", "weight": w_E, "delay": syn_params["delay"]})
        nest.Connect(stim_gen, sub_pops[k]["inh"], "all_to_all",
                    {"synapse_model": "static_synapse", "weight": w_E, "delay": syn_params["delay"]})
...
sim_params = {
    ...
    "stim_duration": 200.0 # Duration of each stimulus step (ms)
}
...
np.savez(
    f"results/output_ce_{sim_id}.npz",
